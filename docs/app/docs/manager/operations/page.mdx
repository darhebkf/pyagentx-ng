...
```python
async def get_many(self, *oids: str, raise_exceptions: bool = True) -> list[Value]
```
...
By default, `NoSuchObjectError` / `NoSuchInstanceError` / `EndOfMibViewError` are raised
if any varbind in the response is an exception value. Pass `raise_exceptions=False` to
return them as `Value.NoSuchObject()` etc. instead, enabling per-varbind inspection:

```python
values = await mgr.get_many(
    "1.3.6.1.2.1.1.1.0",
    "1.3.6.1.2.1.99.99.0",  # does not exist
    raise_exceptions=False,
)
# values[0] = Value.OctetString(...)
# values[1] = Value.NoSuchObject()
```
...
Set a single OID value (v2c/v3 only).
...
Set multiple OID/value pairs in a single SNMP SET PDU (v2c/v3 only). Use this when multiple values must be set atomically — for example, USM user creation with `createAndGo`.
...
await mgr.set_many(
    ("1.3.6.1.2.1.1.5.0", Value.OctetString(b"new-hostname")),
    ("1.3.6.1.2.1.1.6.0", Value.OctetString(b"Server Room")),
)
```
